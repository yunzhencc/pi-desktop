...
import type { AgentSession, AgentSessionEvent, AgentSessionRuntime, CreateAgentSessionOptions, ExtensionCommandContextActions, ExtensionFactory, ExtensionUIContext, ExtensionUIDialogOptions, ExtensionWidgetOptions, ModelRuntime, SessionInfo } from '@earendil-works/pi-coding-agent';
import type { ExtensionUiState } from './extension-ui-state.ts';
import type { SessionFileCatalogStorage } from './json-catalog-store.ts';
import type { PiCreateAgentSessionOptions } from './npm-package-fallback.ts';
import type { LeaseIdentity } from './session-lease.ts';
import type { SessionSchemaInfo } from './session-schema.ts';
import type { SessionTranscriptItem, SessionTranscriptMessage } from './transcript.ts';
import type { CreateSessionOptions, ForkSessionOptions, ForkSessionResult, HostUiRequest, HostUiResponse, NavigateSessionTreeOptions, NavigateSessionTreeResult, RuntimeCommandRecord, SessionCatalogSnapshot, SessionConfig, SessionDriverEvent, SessionEventListener, SessionMessageDeliveryMode, SessionMessageInput, SessionModelSelection, SessionQueuedMessage, SessionRef, SessionSnapshot, SessionStatus, SessionTreeNodeSnapshot, SessionTreeSnapshot, Unsubscribe, WorkspaceCatalogSnapshot, WorkspaceId, WorkspaceRef } from './types.ts';
...
import { isMissingFileError, JsonCatalogStore } from './json-catalog-store.ts';
import {
  createAgentSessionRuntimeWithNpmFallback,

} from './npm-package-fallback.ts';
import { normalizeRuntimeCommandName, skillCommandName } from './runtime-command-utils.ts';
import {
  buildOwnLease,
...
export interface PiSdkDriverOptions {
  ...
  readonly generateThreadTitleOverride?: (
    ...
    options: import('./thread-title-generator.ts').GenerateThreadTitleOptions,
...
export class SessionSupervisor {
  ...
  async setSessionModel(sessionRef: SessionRef, selection: SessionModelSelection): Promise<void> {
    ...
    if (!auth) {
      throw new Error(`No auth configured for ${selection.provider}.`);
    }

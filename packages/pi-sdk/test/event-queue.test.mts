...
import { chainRecoveringEventQueue, singleFlight } from '../src/session-supervisor-utils.ts';

it('event queue keeps delivering after a work item throws', async () => {
...
it('singleFlight runs the factory once for concurrent callers and yields one result', async () => {
...
it('singleFlight clears the entry on failure and lets the next call retry', async () => {
...
it('distinct keys run their own factory concurrently', async () => {

import type { LeaseInfo, LeaseSnapshot } from '../src/session-lease.ts';
...
it('lease path is a .lease sibling of the session file (ignored by pi\'s .jsonl discovery)', () => {
...
it('isSameHolder matches on pid + hostname only', () => {
...
it('same-host lease is dead when its pid is gone, alive when pid runs and mtime fresh', () => {
...
it('cross-host lease falls back to TTL: fresh mtime alive, stale mtime dead', () => {
...
it('leaseBlocksBinding: own lease never blocks, foreign+alive blocks, foreign+dead does not', () => {
...
it('defaultIsPidAlive: current process alive, invalid pids not alive', () => {
...
it('write/read/remove lease round-trips and tolerates missing + corrupt files', async () => {
...
it('stale takeover: a dead foreign lease can be overwritten by our own', async () => {
  await withTempDir(async (dir) => {
    const leasePath = join(dir, 'session.jsonl.lease');

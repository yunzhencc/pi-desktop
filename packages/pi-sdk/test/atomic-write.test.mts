...
it('writes new content and creates missing directories', async () => {
...
it('leaves only the target file behind, no lingering temp files', async () => {
  await withTempDir(async (dir) => {
...
it('concurrent writes never collide or leave a partial file; result is one of the inputs', async () => {
  await withTempDir(async (dir) => {
    const target = join(dir, 'catalog.json');

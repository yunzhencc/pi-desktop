...
export function ProviderDetail({
  ...
  return (
    <article className="flex min-h-0 min-w-0 flex-1 flex-col gap-4 p-4">
      <header className="flex items-center gap-3">
        <ProviderMark provider={provider} />
        <div>
          <h2 className="m-0 text-base font-semibold">{provider.name}</h2>
      ...
      <div className="flex flex-col gap-2.5 border-t border-border-subtle pt-4">
        <div className="settings-provider-nav-title text-xs font-semibold tracking-[0.06em] text-text-tertiary uppercase">{formatMessage({ id: 'providers.auth' })}</div>
        {provider.authType === 'oauth'
          ? <button className="w-fit rounded-md bg-foreground px-3 py-1.75 text-background disabled:opacity-[0.55]" disabled={busy === 'login-openai-codex'} onClick={onLoginChatGPT} type="button">{formatMessage({ id: 'providers.chatgpt.login' })}</button>
        ...
        {provider.authType === 'api_key' && provider.configured && onRemove && (
          <button className="w-fit self-start rounded-md border border-[color-mix(in_srgb,#ef4444_35%,transparent)] bg-transparent px-3 py-1.75 text-[#ef4444] disabled:opacity-[0.55]" disabled={busy === `remove-${provider.id}`} onClick={onRemove} type="button">
      ...
      <div className="flex flex-col gap-2.5 border-t border-border-subtle pt-4">
        <div className="settings-provider-nav-title text-xs font-semibold tracking-[0.06em] text-text-tertiary uppercase">{formatMessage({ id: 'providers.modelSettings' })}</div>
        {!provider.primary && (
          <button className="w-fit rounded-md bg-foreground px-3 py-1.75 text-background disabled:opacity-[0.55]" disabled={!provider.configured || busy === `primary-${provider.id}`} onClick={onSetPrimaryProvider} type="button">{formatMessage({ id: 'providers.setPrimary' })}</button>
        )}

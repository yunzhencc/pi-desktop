...
import { clampRightPanelWidth, readRightPanelWidth, shouldCloseRightPanel } from './right-panel';
...
export function RightPanelResizeHandle({
  ...
  useEffect(() => {
    ...
    const getWidth = (event: PointerEvent) => {
      ...
      return drag && clampRightPanelWidth(drag.startSize - event.clientX + drag.startPosition, mainContentWidth);
    };
  ...
  const reset = () => {
    const nextWidth = readRightPanelWidth(null, mainContentWidth);
    onWidthChange(nextWidth);
    onResizeEnd(nextWidth);
  };
  ...
  return (
    <div
      ...
      tabIndex={0}
    >
      <div className="pointer-events-none m-auto h-full w-px bg-gradient-to-b from-transparent via-foreground/25 to-transparent opacity-0 group-hover:opacity-100 group-active:opacity-100 group-focus-visible:opacity-100" />
    </div>
  );
}

...
const updaterEntry = join(appPath, 'Contents', 'Resources', 'app.asar', 'node_modules', 'electron-updater', 'out', 'main.js');

if (!existsSync(executable))
  throw new Error('未找到 macOS unpacked 安装包，请先运行 pnpm build:unpack。');

for (const entry of [runtimeEntry, updaterEntry]) {
  execFileSync(executable, ['-e', `import(${JSON.stringify(pathToFileURL(entry).href)})`], {
